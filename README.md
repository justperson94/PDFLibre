...
|---------|-------------|------|
| **PDF → Image** | Convert pages to JPEG, PNG, WebP, TIFF, BMP, or GIF with custom DPI and quality | 페이지를 이미지로 변환 (DPI/품질 조절) |
...
| macOS 11+ | ✅ | ✅ |
| Windows 10+ | ✅ | ⏳ pending |
| Linux (Ubuntu 20.04+) | ✅ | ⏳ pending |

> Currently verified on macOS only. Windows / Linux builds are wired up but haven't been runtime-tested yet — expect rough edges until the first cross-platform release. Bug reports on those platforms are especially welcome.
>
> 현재 macOS에서만 동작 확인됨. Windows / Linux 빌드는 구성돼 있지만 아직 실기기 검증 전. 첫 크로스플랫폼 릴리즈 전까진 거친 부분 있을 수 있음.
